...
progres.src.html     ZDROJ — jediné místo k editaci (382 270 B, stav v48)
proxy_shim.js        vrstva sdílené AI, mimo zdroj (10 153 B / 9 822 znaků, stav v47)
...
- Testy: blok I v `test_v54.mjs` (37 kontrol) — max_tokens bez textu → hláška limitu; jen thinking + end_turn → pojistka; normální text → žádný warn (pozitivní protějšek). Regrese 14 sad, 0 chyb. 🚨 Změna těla requestu (max_tokens) → **živě ověřeno 4. 8. 2026:** `asstApi` s jedinou zprávou přes Worker vrátil „OK“, `end_turn`, `claude-sonnet-5`, 8 347 vstupních / 4 výstupní tokeny, 2,8 s — `max_tokens: 6000` prošel validací API i stropem Workeru (`MAX_TOKENS_CAP=8192`). V živém HTML je `max_tokens: 6000` právě 1×. Headless průchod s 10 fotkami + Karlovy tortilly = zbývá na Karlovi z telefonu.
...
- ⚠️ Vedlejší nález: sentinel „nová složka" z v54 se do zdroje zapsal jako **skutečný NUL bajt** (`\x00` v Edit řetězci) — fungoval, ale dělal ze zdroje „binary file" pro grep. Nahrazen `"__newfld__"`.
- Testy: blok H v `test_v54.mjs` (33 kontrol) — 4× AbortController ve zdroji, signal v shimu, chatBusy toast funkčně, abort → česká hláška, signal reálně dorazí do fetch přes shim swap. Regrese 14 sad, 0 chyb. **Živě ověřeno 4. 8. 2026 skutečnými voláními přes novou vrstvu:** `aiCall` (přímý POST na Worker s AbortController) vrátil „OK“ za 1,7 s; `asstApi` (podvržený fetch s propagací `signal`) vrátil „OK“, `end_turn`, `claude-sonnet-5`, 2,9 s — timeout requesty neruší, whitelist drží. Živé ověření původního scénáře = Karel pošle znovu 10 fotek.
- ⚠️ **Dovětek k NUL nálezu (4. 8.):** jeden doslovný NUL bajt zůstal i v TOMHLE dokumentu (v odstavci, který chybu popisoval) — soubor byl pro grep „binary“ a **kontrola tajemství nad `claude/` ho tiše přeskakovala**. Nahrazen viditelným zápisem `\x00`. Poučení: text o NUL bajtu umí NUL přenést; po podobné zmínce ověřit počet NUL v souboru pythonem, grep sám nic neřekne.
...
- 🆕 **Datový model:** `state.favFolders[]` a `state.recFolders[]` (pole názvů, řazené česky) + `folder:""` na položkách `favorites[]`/`recipes[]`. Obojí v `mergeState` výčtu polí → jde do zálohy. Prázdná složka přežívá (seznam je perzistentní, ne odvozený).
- 🆕 **`openPick(title,opts,cb)` + `#pickBg`** — obecný výběrový overlay (vzhled dle `#promptBg`); `folderAssignPick` nad ním staví výběr složky (✓ aktuální / 📁 ostatní / Bez složky / ＋ Nová složka → `openPrompt`).
- **UI (stejné v Oblíbených i Receptech):** chipy `.fld-chips` (`folderChips`) — **Vše / 📁 složky / ＋**, u vybrané složky navíc **✎** (přejmenování se propíše do položek) a **🗑** (smazání položky odzařadí, nemaže). Chip filtruje; při „Vše" se seznam seskupí (nezařazené první, pak `group-label` 📁 po složkách). U položky tlačítko **📁** (`data-fldfood`/`data-fldrec`). Filtr (`favFolderSel`/`recFolderSel`) je jen v paměti, řazení `favSort` z v52 platí uvnitř skupin. CSS: `.foodchip.on` aktivní stav.

### ⚡ v53 — koláč ukazuje skutečné kcal, všechny míry v pásu, den v týdnu v historiích
- **Střed koláče „Dnešní makro" = skutečný součet kcal dne** (`k.kc`) — stejné číslo jako nadpis okna a hero kruh. 📜 Dřív tam byl přepočet z maker (B·4+S·4+T·9), který se od zapsaných kcal lišil a Karel to správně hlásil jako nesoulad. Rozložení koláče a % v legendě dál jedou z kalorií maker. Den s kcal bez maker ukáže „X kcal bez zapsaných maker" místo prázdného stavu.
- **„Míry v čase" ukazují všech 7 měr** — s daty první (grafy), bez dat okno s výzvou „Zapiš míru … a graf se objeví". Řeší Karlovo „chybí ramena": v kódu chyba nebyla, okno se jen neukazovalo bez záznamu v období.
- 🆕 **`fmtDateDow(iso)`** (`DOWCZ`, přes `T12:00:00` proti DST) — den v týdnu v historii jídel (group-label), historii tréninků (`.dt`, obě varianty s/bez názvu) a deníku měr Těla. Kalendáře ho nepotřebují (den v týdnu mají vizuálně).

**Testy várky v53–v54:** 🆕 `test_v54.mjs` (23 kontrol): A koláč (kcal 500 vs. makra 370 — střed 500; den bez maker), B den v týdnu v obou historiích, C 7 oken měr vč. Ramen bez dat, D složky jídel **celou cestou přes UI** (＋ chip → prompt → 📁 → pick → filtr → smazání složky odzařadí), E složky receptů. **Negativně ověřen proti v52** (spadne). Upraven blok F v `test_v52` (pás má nově 7 oken). Regrese **14 sad 2× po sobě, 0 chyb**. Bez API změn.

### ⚡ v52 — řazení Oblíbených + poznámky tréninku v obou kalendářích
**Várka v49–v52 mimo Design (skill `uprava-progres`, Cowork), 8 zadání od Karla se screenshoty. SW bump jen jednou (v52.0), čtyři sekce níže.**

- **Oblíbená jídla:** segment **A–Z / Nové / Nejčastější** (`#favSortSeg`). „Nejčastější" se počítá z historie `state.foods` přes `favKey` (bez migrace), „Nové" podle 🆕 `ts` (Date.now() při přidání — starší položky bez `ts` padají na konec). Volba se drží v 🆕 `profile.favSort`. Kategorie/složky Karel odložil na příští várku.
- **Kalendáře ukazují název tréninku a poznámku** (např. ve které posilovně byl bratr): hlavní kalendář na Přehledu (`.cal-detail`) i týdenní kalendář v Tréninku (`.wk-det` v `renderTrainWeek`) — druhý jmenovaný doplněn po Karlově kontrole náhledu, původně jsem ho minul.

### ⚡ v51 — míra Ramena + „Míry v čase" (graf pro každou partii)
- 🆕 **Míra Ramena** (`shoulders`): `MEASURES` (7 položek, index 3), formulář deníku Těla (pole přeskládaná: Biceps/Hrudník/Ramena · Pas/Boky/Stehno · Lýtko), všechny šablony `state.body` záznamu, reset formuláře a `zapis_telo` asistenta (parametr `ramena`). Staré záznamy a zálohy mají `shoulders` prostě nevyplněné — nic se nemigruje.
- 🆕 **Karta `#card-meascharts` „Míry v čase"** ve Statistikách Těla + `chartMeasures()`: swipe pás (`#measSwipe`/`#measDots`), jeden čárový graf na každou míru s daty v období (`periodCut()`), grafy `chMeas0..N`, pod grafem změna za období barevně podle směru (`dir` z MEASURES — u pasu a boků je dobrý pokles). Jediné měření = graf + výzva místo změny. Zapojeno do `STATS_CARDS.telo` a `renderStatsFor`.

### ⚡ v50 — všechny rekordy se swipe, objem partií v plných barvách
- **Osobní rekordy:** místo `slice(0,6)` VŠECHNY cviky, stránkované po 6 dlaždicích (`#prSwipe`/`#prDots`, konstanta `PR_STR=6`); do 6 rekordů zůstává obyčejná mřížka bez swipe. Řazení i zvýraznění PR beze změny.
- **Týdenní objem podle partií:** zrušen dataset „Ostatní" (dřív top 5 + šedý zbytek). Každá partie má **stálou barvu** přes `colOf`: Záda s1, Hrudník s2, Nohy s3, Ramena s4, Triceps s5, Biceps s6, Břicho `#ffd60a`, Celé tělo `#ac8e68`, fallback muted. Barva partie je stejná napříč týdny i obdobími.

### ⚡ v49 — datum neroztlačuje formulář + jednotky síly podle skutečných zápisů
- **Layout:** iOS date input má vnitřní minimální šířku a roztahoval `.row-3` mřížku (Datum přetékal do Typu — Karlův screenshot). Oprava: `.row-3>.field{min-width:0}` + `width:100%` na input/select. Platí pro všechny `.row-3` formuláře.
- 🔴 **Oprava jednotek ve statistice síly:** `exUnitByName` dřív brala jednotku z DEFINICE cviku v DB (a až pak ze záznamů) — cvik vedený v DB jako „kg", ale zapisovaný na opakování, ukazoval „15 kg" u 15 opakování. Nově rozhoduje **nejnovější skutečný záznam**, pak DB, pak „kg". Navíc `dailyMax`, `dailyVol` i `renderPrTable` **nemíchají záznamy s jinou jednotkou** do jedné řady/rekordu (u PR se jednotka cviku určuje z nejnovějšího záznamu — starý kg zápis nepřebije aktuální opakovací a naopak).

**Testy várky v49–v52:** 🆕 `test_v52.mjs` (34 kontrol): S statika, A/B jednotky síly (smíšený cvik kg→reps end-to-end), C rekordy (8 cviků = 2 stránky; ≤6 bez swipe), D objem (7 datasetů, žádný „Ostatní", 7 různých barev), E ramena (formulář + `zapis_telo` + MEASURES), F grafy měr (2 míry = 2 okna + canvasy + tečky), G řazení Oblíbených (všechna tři řazení kliky + persistence), H poznámky v obou kalendářích. **Ověřeno negativně proti v48** (spadne). Regrese **13 sad 2× po sobě, 0 chyb** (a znovu po dodatečné opravě týdenního kalendáře). Bez API změn — živé volání není potřeba.

### ⚡ v48 — „Dnešní makro" = 4 dny se swipe a plnou úpravou jídelníčku
**Várka mimo Design (skill `uprava-progres`, poprvé z Coworku), zadání od Karla: „Vidět 3 dny zpátky a upravovat jídelníček na všech dnech i zpětně jako v historii — swipe okna, okno stejné i s jídlem."**

- Karta `#card-today` = swipe pás **`#todaySwipe`** + tečky **`#todayDots`**, **4 okna** (Dnes / Včera / −2 / −3; `TODAY_SWIPE_DNU=4`). Recyklovaná mechanika `.swipe`/`.swipe-card` z v25/v43.
- Každé okno: nadpis se **skutečným součtem kcal dne**, koláč maker ve stylu původní karty (`chMacroDay0..3`, `pie-center` = kcal z maker), seznam jídel dne a **stejné akce jako Historie jídel**: ★ oblíbené (`data-tdfav`), **✎ upravit** (`data-tdedit` → `openFoodEdit` z v42: název/kcal/makra/datum), ⧉ na talíř (`data-tdcopy` + `showSolo` na Talíř), 🗑 smazat s potvrzením a archivem (`data-tddel`).
- **＋ Přidat jídlo — [den]** (`data-tdadd`): předvyplní datum na Talíři (`fdate` + `fdateTouch`, mechanika zpětného zápisu z v27/v45) a otevře kartu Talíř.
- **`#macroPieBox`, `#foodList` a graf `chMacro` ODSTRANĚNY** — dnešní seznam žije v okně Dnes (starý blok v `renderCalories` vyřazen). `renderMacroPie()` zůstala jménem, kreslí celý pás; **scroll pozice se při překreslení drží** (`keep=sw.scrollLeft`), takže úprava včerejška nevrací pás na Dnes.
- Hero kruh kalorií nahoře na záložce Jídlo zůstává **vždy dnešní** (záměr).

### ⚡ v47 — AI generace: sonnet-5 všude, foto jídla na opus-5
**Zadání od Karla: „Rozpoznávání jídla z fotky je nepřesné a stejné fotky vyhodnotí pokaždé úplně jinak — připojit lepší model."**

- Default `claude-sonnet-4-5` (rok stará generace) → **`claude-sonnet-5`** na VŠECH místech: `aiCall`, `aiFoodSearch`, `asstApi`, fallback při ukládání klíče v `renderApiKeyBox` — a **v shimu**, který v proxy režimu posílal model natvrdo.
- 🆕 **`aiCall(content,maxTokens,model)`** — třetí parametr; `aiAnalyzeFood` jím vynucuje **`claude-opus-5`** (nejsilnější vidění). ~5× dražší než sonnet, řádově desetihaléře za fotku; strop $10/měs. na Workeru jistí. Shim třetí parametr předává.
- **Prompt foto analýzy dostal pevnou metodiku**: 1) složky → 2) gramáž podle referencí ve scéně (talíř 26 cm, vidlička 20 cm, hrnek 250 ml…) → 3) tabulkové hodnoty na 100 g; „při nejistotě STŘEDNÍ odhad — stejná fotka musí dát stejný výsledek"; započítat vstřebaný olej/dresink. Determinismus úplný nebude (`temperature` je zakázaná — viz v38), rozptyl stlačuje prompt + silnější model.
- 🆕 migrace v `mergeState`: uložený `aiModel` generace 4 (`/^claude-(sonnet|haiku|opus)-4/`) → `""` = nový default; novější hodnoty se nechávají. Kryje uživatele s vlastním klíčem i staré zálohy.
- **Worker v2.1** (`proxy/worker.js`): 🆕 `DEFAULT_ALLOW="claude-sonnet-5,claude-opus-5"` jako fallback, když `MODEL_ALLOW` není nastavená; `DEFAULT_MODEL` bump. 🔴 **Živý Worker se neredeployoval — místo toho Karel nastavuje proměnnou `MODEL_ALLOW=claude-sonnet-5,claude-opus-5`** (Cloudflare → progres-ai → Settings → Variables and Secrets, typ Text). **Bez toho Worker oba modely tiše přepíše na starý default** (whitelist logika: nepovolený model → `models[0]`).
- 🚨 **Změna těla requestu → po nasazení JEDNO skutečné volání** (mock testy serverovou validaci nevidí — poučení z v38). **Proběhlo 2. 8. 2026, OK:** na živém webu ve sdíleném režimu (přes `workers.dev`) `aiCall(..., 50, "claude-opus-5")` → skutečný model `claude-opus-5`, odpověď „OK"; `aiCall(..., 50)` bez modelu → skutečný model `claude-sonnet-5`, odpověď „OK". Potvrzuje najednou: whitelist na Workeru je aktivní, výchozí model je sonnet-5 a třetí parametr `aiCall` projde validací API. ⚠️ **Pozor na výklad: kód Workeru na Cloudflare zůstává v2.0** (`/health` hlásí `version: 2.0`) — whitelist zajišťuje **proměnná `MODEL_ALLOW`**, kterou Karel nastavil v dashboardu. `worker.js` v2.1 (s fallbackem `DEFAULT_ALLOW`) leží zatím jen v gitu a nahraje se při příštím skutečném zásahu do Workeru. **`MODEL_ALLOW` proto nemazat** — bez ní by živý v2.0 přepsal oba modely na starý default.

### ⚡ v46 — chat asistenta bere 10 fotek
**Zadání od Karla: „Možnost přidat v chatu alespoň 10 fotek u AI asistenta."**

- 🆕 **`CHAT_IMG_MAX=10`** v `initAsst` (bylo natvrdo 3) + hláška při překročení („Vejde se 10 fotek — beru N" / „Máš už 10 fotek"). Náhledy `.chat-imgs` se lámou do řádků (flex-wrap tam už byl).
- Fotky se dál zmenšují na 1024 px JPEG → při 10 fotkách ~2–3 MB request, bezpečně v limitech API i Workeru; **do historie vlákna se ukládá jen počet fotek, ne data** (žádný bloat localStorage — chování z v34 beze změny).
- Limit u **foto jídla zůstává 3** — zadání se týkalo jen chatu.

**Testy a prostředí várky v46–v48:**
- 🆕 **`test_v48.mjs`** (39 kontrol, jedna sada pro celou várku): S = statika buildu (modely, `CHAT_IMG_MAX`, žádný `#foodList`/`#macroPieBox` + pozitivní protějšky), A–D = swipe okna (rozmístění jídel po dnech, kcal v nadpisu, ✎ editace včerejška end-to-end přes `#foodEditBg`, ＋ přidání na −2 dny až po zápis přes formulář, 🗑 smazání s archivem), E = fotky (pozitivně 2, limitně 13→10), F = **modely v requestu chycené za shimem** (opus u fotky, sonnet-5 default, absence `temperature`), G = migrace `aiModel`. **Ověřeno negativně proti nasazené v45** (spadne). Regrese **12 sad 2× po sobě, 0 chyb**.
- Vzniklo v **Cowork session** (první deploy-příprava odsud): Playwright headless-shell se stahoval po kouscích (`curl -C -`, 45s limit shellu), chybějící `libXdamage` vyřešena přes `dpkg -x` + `LD_LIBRARY_PATH`, testy běžely po dávkách. **Git kroky (commit/push) běžely u Karla v terminálu**; po session zbyly k ručnímu smazání `index.lock` a `_rmtest`.
- ⚠️ **Dodatek z téže session (po uzavření várky):** původní diagnóza „běžíme v cloudu, most neumí mazat" byla **chybná** — session běžela na počítači (`local_` cesty, arm64 sandbox) a mazání blokovala **bezpečnostní pojistka Cowork sandboxu**, která se odemyká nástrojem `allow_cowork_file_delete` (Karel schválí kliknutím). Po povolení git commit + hooky ze session fungují bez zámků (ověřeno commitem téhle opravy). **Push ze sandboxu nejde** — SSH klíč je jen v `~/.ssh` na Macu. Starý obchvat `mv` zámku nemohl fungovat nikdy (přesun ven ze složky je taky mazání). CLAUDE.md sekce „Dvě prostředí" přepsána podle skutečnosti.
- ⚠️ `tools/make_preview.py`: dočasný soubor přes `tempfile.mkdtemp` — pevné `/tmp/_preview_base.html` kolidovalo se souborem cizí session (jiný UID, nešlo smazat ani přepsat).
...
**Trénink:** **od v44 bez AI plánu** — plán sestaví asistent (má nástroj `uloz_sablonu`); zkratka Nastavení drží, jak trénuješ (týdně, zkušenost, vybavení, cviků), a asistent z toho čte. **Od v42 jde zpětně upravit zaznamenaný trénink** — ikona posuvníků v historii načte záznam do formuláře včetně sérií a vah, uložení přepíše původní. Dál: typ Fitko / Kardio / Jiné; týdenní přehled (**od v52 s názvem a poznámkou tréninku u vybraného dne**) + Statistiky (síla, objem cviku, rekordy — **od v50 všechny, swipe po 6**, objem partií — **od v50 každá svou stálou barvou, bez „Ostatní"**, **heatmapa konzistence za 20 týdnů**; **od v49 jednotky podle skutečných zápisů** — cvik na opakování už neukazuje kg). Fitko = partie + cviky se sériemi (modré zvýraznění nevyplněného; steppery s držením, krok u kg 0,5; pauza; koš s potvrzením); výběr cviku přes picker; databáze + vlastní; **šablony s detailem, načtením a přejmenováním**; kroky; AI plán; historie **s možností pojmenovat trénink**.  Kardio = aktivita + kruhový číselník + slider metriky.
**Tělo:** **od v45 zkratka Můj profil** (pohlaví, věk, výška, cílová váha; aktuální váha se bere z posledního vážení). **Od v51 míra Ramena** a ve Statistikách karta **„Míry v čase"** — swipe graf pro každou míru s daty. Dál: přehled váhy + míry (kruhové dlaždice `#bodyOverview`) + Statistiky; hero váha; míry-seznam; detailní záznam; fotky + porovnání.
**Kalorie (Talíř):** **od v43 jde rozložení maker prohlížet po jednotlivých dnech** (swipe pás: průměr za období + posledních 7 dní, v nadpisu kcal daného dne). **Od v42 jde zpětně upravit zapsané jídlo** — název, kalorie, makra i datum. Dál: slož jídlo z ingrediencí a „Sníst" (i zpětně na jiný den — s varováním a tlačítkem Dnes); databáze ~313 + online OFF + hledání přes AI s přístupem na web; sken kódu; AI foto — až 3 fotky, popis, jistota odhadu a rozpis ingrediencí (**od v47 na `claude-opus-5`**, zbytek AI na `claude-sonnet-5`); Recepty (**od v54 se složkami**); Oblíbené (**od v52 řazení A–Z / Nové / Nejčastější, od v54 složky** — chipy, 📁 u položky, ✎/🗑 u vybrané složky); **Dnešní makro — střed koláče od v53 = skutečné kcal dne**; **Dnešní makro — od v48 swipe okna 4 dnů (Dnes/−1/−2/−3) s koláčem, jídly dne, úpravou ★✎⧉🗑 a přidáním jídla na daný den**; Historie jídel; **Statistiky nově včetně rozpadu maker, maker v čase, bílkovin, dnů v týdnu, tabulky dnů a top jídel**.
...
- **Když Karel pošle úpravu z Claude Design:** viz skill `nasazeni-progres-z-design`. (V nové session **nejdřív `git -C repo pull` a ověřit `cmp`**.) `diff` proti výstupu `build.py --no-shim` **a proti uploadu s odstřiženým shimem** → ověřit helpery **i kotvy shimu** → `patch -U 6` do `progres.src.html` (offset −18) → build → **cmp/MD5 shoda** → **vrátit vlastní úpravy, které export smazal** (v37+v38 aparát!) → testy (0 chyb) → deploy. SW vlastní řadou (v14…v48), ne číslem z CD sw.js (⚠️ Design měl naposledy shodou okolností taky v47.0 — nenechat se zmást, řady spolu nesouvisí).

## Roadmapa
- ✅ ~~Testy neexistují~~ → **hotovo: runner `tools/run_tests.mjs` (obě konvence hlášení) + 12 sad** (`test_backup31`, `test_deleni`, `test_regress29`, `test_v38`–`test_v45`, `test_v48`), stav k v48. Zaniklé sady z éry kontejneru se dopisují jen, když se ukáže díra.
