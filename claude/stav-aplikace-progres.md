...
- 🚨 **Změna těla requestu → po nasazení JEDNO skutečné volání** (mock testy serverovou validaci nevidí — poučení z v38). **Proběhlo 2. 8. 2026, OK:** na živém webu ve sdíleném režimu (přes `workers.dev`) `aiCall(..., 50, "claude-opus-5")` → skutečný model `claude-opus-5`, odpověď „OK"; `aiCall(..., 50)` bez modelu → skutečný model `claude-sonnet-5`, odpověď „OK". Potvrzuje najednou: Worker na Cloudflare je nasazený s novým whitelistem, výchozí model je sonnet-5 a třetí parametr `aiCall` projde validací API.
