...
- Vzniklo v **Cowork cloud session** (první deploy-příprava odsud): Playwright headless-shell se stahoval po kouscích (`curl -C -`, 45s limit shellu), chybějící `libXdamage` vyřešena přes `dpkg -x` + `LD_LIBRARY_PATH`, testy běžely po dávkách. **Git kroky (commit/push) běžely u Karla v terminálu** — most cloudu neumí mazat soubory, git by tu nechával zámky (`.git/index.lock`); po session zůstaly k ručnímu smazání `index.lock` a `_rmtest`.
